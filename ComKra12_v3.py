...
and evaluate sequences that are annotated in the 2nd run but have taxids 
not present in the provided df1 reference file.
...
------------------------------
- Modified to evaluate sequences that are annotated in out2 but not in df1.
...
def parse_args():
    ...
    p.add_argument('--threads', type=int, default=1, help='Number of worker threads to use for processing titles (default: 1)')
...
def main():
    ...
    # parse out2 first and identify annotated sequences not in df1
    ...
    for title, rec in out2_records.items():
        ...
        if (status and status.startswith('C')) or (taxid != '0' and taxid != 'NA'):
            ...
            if taxid in df1_taxids:
                ...
                not_exist_in_df1 += 1
                # Add to target list since it's annotated but not in df1
    ...
    logging.info('Out2: annotated sequences not in df1 to review: %d', len(target_titles))
    ...
    if not target_titles:
        logging.info('No annotated sequences in out2 that are not in df1. Exiting.')
        logging.info('Wrote empty output %s', outpath)
        return

    # Prepare sqlite index for out1 (streaming approach)
    ...
    if args.threads == 1:
        logging.info('Processing %d annotated titles not in df1 sequentially...', len(target_titles))
        with open(outpath, 'a', encoding='utf-8') as outfh:
            ...
            for title in target_titles:
                ...
                processed += 1
                if processed % 1000 == 0:
                    logging.info('Processed %d / %d annotated sequences not in df1', processed, len(target_titles))
    else:
        max_workers = args.threads
        logging.info('Processing %d annotated titles not in df1 using %d threads...', len(target_titles), max_workers)
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
            # executor.map preserves input order; iterate results and write sequentially
            with open(outpath, 'a', encoding='utf-8') as outfh:
                writer = csv.writer(outfh, delimiter='\t', lineterminator='\n')
                for i, row in enumerate(ex.map(worker, target_titles), start=1):
                    writer.writerow(row)
                    if i % 1000 == 0:
                        logging.info('Processed %d / %d annotated sequences not in df1', i, len(target_titles))
                processed = i
    ...
    logging.info('Finished. Processed %d annotated sequences not in df1. Results written to %s', processed, outpath)
